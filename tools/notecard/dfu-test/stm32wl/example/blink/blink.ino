...
void loop()
{
	...
	pinMode(D9, OUTPUT);								// USART1_TX PA9
	digitalWrite(D9, (count & 1) != 0 ? HIGH : LOW);	// TX IS FASTER ON SCOPE
	pinMode(A2, OUTPUT);								// USART1_RX PA10
	digitalWrite(A2, (count & 2) != 0 ? HIGH : LOW);	// RX IS SLOWER ON SCOPE
	delay(LOOP_DELAY);
}
